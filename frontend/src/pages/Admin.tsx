...
export default function Admin() {
  ...
  const [signatureSchema, setSignatureSchema] = useState({
    function_name: "solve",
    return_type: "integer",
  ...
  const fetchProblems = async () => {
    try {
      const res = await fetch('http://localhost:8000/problems/', { headers: { 'Authorization': `Bearer ${token}` } });
  ...
  const handleCreateProblem = async () => {
    ...
    try {
        ...
        // Refresh problems
        const pRes = await fetch('http://localhost:8000/problems/', { headers: { 'Authorization': `Bearer ${token}` } });
        const pData = await pRes.json();
        setProblems(pData);
        
        // Reset form & go to testcases
        setNewTitle(""); setNewDescription(""); setNewTopics("");
        setSignatureSchema({ function_name: "solve", return_type: "integer", parameters: [] });
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:8000/admin/problems/${selectedProblem}/generate-testcases`, {
        method: 'POST',
        headers: { 
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const handleSaveChecker = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:8000/admin/problems/${selectedProblem}/checker`, {
        method: 'PUT',
        headers: { 
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
      ...
      alert(`Error saving checker: ${err.message}`);
    } finally {
    ...
    <div className="flex flex-col h-[calc(100vh-64px)] overflow-hidden bg-[#1e1e1e] text-gray-200">
      <div className="bg-gray-800 border-b border-gray-700 flex px-6 py-3 space-x-4">
        <button 
          ...
          className={`px-4 py-2 rounded font-bold transition-colors ${activeTab === 'create' ? 'bg-purple-600 text-white' : 'text-gray-400 hover:text-white'}`}
        >
          Create New Problem
        </button>
        <button 
          onClick={() => setActiveTab('testcases')}
          className={`px-4 py-2 rounded font-bold transition-colors ${activeTab === 'testcases' ? 'bg-purple-600 text-white' : 'text-gray-400 hover:text-white'}`}
        >
        ...
        {activeTab === 'create' ? (
          <div className="w-full max-w-3xl mx-auto p-8 overflow-y-auto">
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-semibold mb-1">Metadata Schema (Function Signature)</label>
                <div className="bg-gray-800 border border-gray-700 rounded p-4 space-y-4">
                  <div className="flex space-x-4">
                    <div className="w-1/2">
                      <label className="block text-xs font-semibold mb-1 text-gray-400">Function Name</label>
                      <input 
                        value={signatureSchema.function_name} 
                        onChange={e => setSignatureSchema({...signatureSchema, function_name: e.target.value})}
                        className="w-full bg-gray-900 border border-gray-700 rounded p-2 focus:outline-none focus:border-purple-500 text-sm" 
                      />
                    </div>
                    <div className="w-1/2">
                      <label className="block text-xs font-semibold mb-1 text-gray-400">Return Type</label>
                      <select 
                        value={signatureSchema.return_type} 
                        onChange={e => setSignatureSchema({...signatureSchema, return_type: e.target.value})}
                        className="w-full bg-gray-900 border border-gray-700 rounded p-2 focus:outline-none focus:border-purple-500 text-sm"
                      >
                  ...
                  <div>
                    <label className="block text-xs font-semibold mb-2 text-gray-400">Parameters</label>
                    {signatureSchema.parameters.map((param, idx) => (
                      <div key={idx} className="flex space-x-4 mb-2 items-center">
                        <input 
                          value={param.name}
                          onChange={e => {
                            const newParams = [...signatureSchema.parameters];
                            newParams[idx].name = e.target.value;
                            setSignatureSchema({...signatureSchema, parameters: newParams});
                          }}
                          placeholder="Name (e.g. nums)"
                          className="w-5/12 bg-gray-900 border border-gray-700 rounded p-2 focus:outline-none focus:border-purple-500 text-sm"
                        />
                        <select 
                          value={param.type}
                          onChange={e => {
                        ...
                        <button 
                          onClick={() => {
                            const newParams = signatureSchema.parameters.filter((_, i) => i !== idx);
                            setSignatureSchema({...signatureSchema, parameters: newParams});
                          }}
                          className="w-2/12 bg-red-600 hover:bg-red-700 text-white rounded p-2 text-sm font-bold"
                        >
                          Remove
                        </button>
                      </div>
                    ...
                    <button 
                      onClick={() => {
                        setSignatureSchema({
                          ...signatureSchema, 
                          parameters: [...signatureSchema.parameters, { name: "", type: "integer" }]
                        });
                      }}
                      className="bg-gray-700 hover:bg-gray-600 text-white rounded px-3 py-1 text-sm font-bold mt-2"
                    >
                      + Add Parameter
                    </button>
                  </div>
          ...
          <>
            <div className="w-1/2 p-6 overflow-y-auto border-r border-gray-800">
              ...
              {result && (
                <div className={`mt-6 p-4 rounded border ${result.type === 'error' ? 'bg-red-900/30 border-red-800 text-red-300' : 'bg-green-900/30 border-green-800 text-green-300'}`}>
